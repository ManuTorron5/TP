...
public class AtaxxMove extends GameMove {
	...
	/**
	 * preRow indicates the row where the piece we want to move is placed at first.
	 * 
	 * <p>
	...
	/**
	 * preCol indicates the col where the piece we want to move is placed at first.
	 * 
	 * <p>
	 * preCol indica la columna en la que se encuentra la pieza que queremos mover.
	 */
	...
	/**
	 * The column where to place the piece returned by {@link GameMove#getPiece()}
	 * .
	 * <p>
	 ...
	 * placed at position ({@code preRow},{@code preCol})
	 * to position ({@code row},{@code col}).
	 * 
	 * <p>
	 * Construye un movimiento para mover una ficha del tipo referenciado por
	 * {@code p} situada en la posicion({@code preRow},{@code preCol})
	 * a la posicion ({@code row},{@code col}).
	 * 
	 * @param preRow
	...
	public void execute(Board board, List<Piece> pieces) {
		...
		} else if((preRow - row > 2 || row - preRow > 2) && (preCol - col > 2 || col - preCol > 2)){
			throw new GameError("The position where you are trying to move is too far");
